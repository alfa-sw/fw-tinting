...
int main(void) {
    ...
    while (1) {
        ...
        // bit9: Jar Detection Microswitch 1
        if (JarPhotocellStatus(JAR_DETECTION_MICROSWITCH_1, FILTER) == TRUE)
            TintingAct.Jar_Photocells_state |= (1L << JAR_DETECTION_MICROSWITCH_1);
        else
            TintingAct.Jar_Photocells_state &= ~(1L << JAR_DETECTION_MICROSWITCH_1);

        // bit10: Jar Detection Microswitch 2
        if (JarPhotocellStatus(JAR_DETECTION_MICROSWITCH_2, FILTER) == TRUE)
        ...
        // Pulizia Puntuale 
        if (Punctual_Cleaning == ON)
            TintingAct.Punctual_Cleaning_Process = TRUE;
        else
            TintingAct.Punctual_Cleaning_Process = FALSE;

        // Ricircolo Puntuale
        if (ricrc_st == ON)
            TintingAct.Punctual_Recirculation_Process = TRUE;
        else
            TintingAct.Punctual_Recirculation_Process = FALSE;

        // Agitazione Puntuale
        if (stirr_st == ON)
            TintingAct.Punctual_Stirring_Process = TRUE;
        else
            TintingAct.Punctual_Stirring_Process = FALSE;

        // Refill
        if (MachineStatus.level == ROTATING_ST)
            TintingAct.Refill_Process = TRUE;
        else
            TintingAct.Refill_Process = FALSE;

        // Erogazione 
        if ((MachineStatus.level == COLOR_SUPPLY_ST) || (Run_Dispensing == 1))
            TintingAct.Dispensing_Process = TRUE;
        else
            TintingAct.Dispensing_Process = FALSE;

        // Pulizia Post Erogazione
        if (Clean_Activation == TRUE)
            TintingAct.Cleaning_After_Dispensing_Process = TRUE;
        else
            TintingAct.Cleaning_After_Dispensing_Process = FALSE;

        // Ricircolo Temporizzato
        if (tinting_ricirc_active == TRUE)
            TintingAct.Temporized_Recirculation_Process = TRUE;
        else
            TintingAct.Temporized_Recirculation_Process = FALSE;

        // Reset
        if (MachineStatus.level == RESET_ST)
            TintingAct.Reset_Process = TRUE;
        else
            TintingAct.Reset_Process = FALSE;

        // Dosaggio Controllato 
        if (Dos_Temperature_Enable == TRUE)
            TintingAct.Temperature_Controlled_Dispensing_Process = TRUE;
        else
            TintingAct.Temperature_Controlled_Dispensing_Process = FALSE;

        // Posizionamento Barattolo
        if (MachineStatus.level == JAR_POSITIONING_ST)
            TintingAct.Jar_Positioning_Process = TRUE;
        else
            TintingAct.Jar_Positioning_Process = FALSE;

        // Canellazione Scrittura EEprom
        if (EEprom_Writing_Erasing == TRUE)
            TintingAct.EEprom_Erase_Write_Process = TRUE;
        else
            TintingAct.EEprom_Erase_Write_Process = FALSE;

        // Movimentazione Tavola
        if (Movimentazione_Tavola == TRUE)
            TintingAct.Positioning_Process = TRUE;
        else
            TintingAct.Positioning_Process = FALSE;

        // Autoapprendimento Tavola
        if (Autoapprendimento_Tavola == TRUE)
